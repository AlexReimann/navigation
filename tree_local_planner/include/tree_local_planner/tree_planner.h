#ifndef TREE_LOCAL_PLANNER_TREE_PLANNER_H_
#define TREE_LOCAL_PLANNER_TREE_PLANNER_H_
...
namespace tree_local_planner {
  ...
  public:
     ...
     */
    TreePlanner(std::string name, base_local_planner::LocalPlannerUtil *planner_util);

    /**
     * @brief  Destructor for the planner
     */
    ~TreePlanner() {if(traj_cloud_) delete traj_cloud_;}

    /**
     * @brief Reconfigures the trajectory planner
     */
    void reconfigure(TreePlannerConfig &cfg);
  ...
  private:
    ...
    double sim_period_;///< @brief The number of seconds to use to compute max/min vels for tree
