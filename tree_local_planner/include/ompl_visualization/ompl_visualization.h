...
public:
  ...
  void visualize_path_states(std::vector<ompl::base::State *>& states, std::string frame_id);
  void add_big_marker(std::string frame_id, int id, double x, double y, double z, float color_r, float color_g,
                      float color_b);

protected:
  visualization_msgs::Marker make_marker(std::string frame_id, int id, double x, double y, double z);
